    ...
    try:
        ...
        try:
            ...
            await bot.send_message(
                ...
                f"✅ *Оплата прошла успешно!*\n\n"
                f"Ваш доступ к мастер-классу открыт.\n\n"
                f"ВАЖНО: после перехода по ссылке ОБЯЗАТЕЛЬНО ПОДПИШИТЕСЬ на канал с мастер-классом, чтобы не потерять к нему доступ и иметь возможность вернуться к просмотру позже!\n"
                f"{safe_general_link}\n\n"
